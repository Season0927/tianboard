...
void mpu_get_data()
{
    ...
    imu.temp = 21 + mpu_data.temp / 333.87f;
	  /* 2000dps -> rad/s */
	  imu.wx   = mpu_data.gx / 16.384f / 57.28578f; 
    imu.wy   = mpu_data.gy / 16.384f / 57.28578f; 
    imu.wz   = mpu_data.gz / 16.384f / 57.28578f;
}
...
uint8_t mpu_device_init(void)
{
	...
	uint8_t MPU6500_Init_Data[10][2] = {{ MPU6500_PWR_MGMT_1, 0x80 },     /* Reset Device */ 
                                      ...
                                      { MPU6500_INT_ENABLE, 0x01}};    /* Enable AUX */ 
	for (i = 0; i < 10; i++)
	{
...
void mpu_offset_call(void)
{
	...
	for (i=0; i<500;i++)
	{
		...
		mpu_data.az_offset += (int16_t)(mpu_buff[4] << 8 | mpu_buff[5]);
	
		mpu_data.gx_offset += (int16_t)(mpu_buff[8]  << 8 | mpu_buff[9]);
		mpu_data.gy_offset += (int16_t)(mpu_buff[10] << 8 | mpu_buff[11]);
		mpu_data.gz_offset += (int16_t)(mpu_buff[12] << 8 | mpu_buff[13]);
...
void imu_attitude_update(void)
{
  ...
  imu.q[3] = q3;
	/* yaw    -pi----pi */
	imu.yaw = atan2(2*q1*q2 + 2*q0*q3, -2*q2*q2 - 2*q3*q3 + 1)* 57.28578f; 
	/* pitch  -pi/2----pi/2 */
	imu.pit = -asin(-2*q1*q3 + 2*q0*q2)* 57.28578f;   
	/* roll   -pi----pi  */	
	imu.rol =  -atan2(2*q2*q3 + 2*q0*q1, -2*q1*q1 - 2*q2*q2 + 1)* 57.28578f;
}
